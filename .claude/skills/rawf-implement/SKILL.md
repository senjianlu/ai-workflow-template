...
5. 逐条执行 plan.md 的全部测试用例,记录真实结果与证据(实际命令 + 关键
   输出)。未执行的写"未执行"+原因,失败的写失败——评审者会在动作边界
   (review-standards.md「评审动作边界」)内重跑可运行的用例,边界外用例
   只核验 evidence/ 证据,虚报通过在评审中是 blocker 级问题。证据落盘:
   边界外用例(build / E2E / 需启动服务类)的**完整原始输出必须**落任务
   目录 `evidence/`(评审者不重跑这类用例,证据缺失即按虚报处理);其余
   体量较大的证据(完整日志、截图)同样落 `evidence/`;引用的素材(Design
   稿、参考图)放 `assets/`。以上均须在**评审前**落盘——`.ai/` 计入
